...
void BackgroundSlicingProcess::process_fff()
{
	...
	if (this->set_step_started(bspsGCodeFinalize)) {
	    if (! m_export_path.empty()) {
			...
			CopyFileResult copy_ret_val = copy_file(m_temp_output_path, export_path, with_check);
			switch (copy_ret_val){
			case CopyFileResult::CopySuccess: break; // no error
			case CopyFileResult::FailCopyFile:
				throw std::runtime_error(_utf8(L("Copying of the temporary G-code to the output G-code failed. Maybe the SD card is write locked?")));
				break;
			case CopyFileResult::FailFilesDifferent:
				...
				break;
			case CopyFileResult::FailRenaming:
				...
				break;
			case CopyFileResult::FailCheckOiginNotOpened:
				...
				break;
			case CopyFileResult::FailCheckTargetNotOpened:
...
void BackgroundSlicingProcess::prepare_upload()
{
	...
	if (m_print == m_fff_print) {
		m_print->set_status(95, _utf8(L("Running post-processing scripts")));
		if (copy_file(m_temp_output_path, source_path.string()) != CopyFileResult::CopySuccess) {
